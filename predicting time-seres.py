...
def ML_training(train_inputs,train_labels):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(dataloader):
            ...
            targets_tensor = torch.stack(targets).transpose(0,1).to(torch.float32)

            loss_value = reg_criterion(outputs, targets_tensor)
            loss_value.backward()
            optimizer.step()
            running_train_loss += loss_value.item()

            print("real:", targets_tensor.detach().cpu().numpy().flatten())
            print("pred:", outputs.detach().cpu().numpy().flatten())
            print("-------")

            if epoch == num_epochs-1:
                ground_truth_values = targets_tensor.detach().cpu().numpy().flatten()
    ...
    plotting_actual_noise(predictions,gt)
...
def plotting_actual_noise(predictions,gt):
    for datapoint in range(0, len(predictions)):
        plt.plot(gt[datapoint],label="original")
        plt.plot(predictions[datapoint],label="predeicted")
        plt.legend()
        plt.savefig(f"{datapoint}_raw.png")
...
def run_ML():
    ...
    if normalization_flag == False:
        Data_Z = find_RMS_noise(Data_O) 
        train_x_norm, val_x_norm, test_x_norm, train_y_norm, val_y_norm, test_y_norm, train_z_norm, val_z_norm, test_z_norm = data_splitting (Data_X, Data_Y, Data_O)
