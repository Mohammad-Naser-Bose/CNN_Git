...
def standarization(train_inputs,val_inputs,test_inputs):
    scaler = StandardScaler()
    train_ready = tensor_to_2d_list(train_inputs)
    train_stack = np.vstack(train_ready)
    scaler.fit(train_stack)
    train_inputs_st=scaler.transform(train_inputs)
    val_inputs_st=scaler.transform(val_inputs)
    test_inputs_st=scaler.transform(test_inputs)
    return train_inputs_st, val_inputs_st, test_inputs_st
def tensor_to_2d_list(tensor_data):
    return [tensor_data.numpy().reshape(-1) for tensor in tensor_data.values()]
def list_to_tensor(list_data, original_shape):
        return{f"{i+1}":torch.tensor(data.reshape(original_shape)) for i, data in enumerate(list_data)}
...
def run_CNN():
    ...
    train_inputs_st,val_inputs_st,test_inputs_st = standarization(train_inputs,val_inputs,test_inputs)
    
    model = ML_train_model(num_freq_comp,num_time_comp,train_inputs_st,train_labels, val_inputs_st, val_labels)
    test_errors = ML_test_model(model, num_freq_comp, num_time_comp, test_inputs_st, test_labels)
