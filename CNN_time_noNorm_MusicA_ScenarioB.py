...
################################### Main
def resampling():
    audio_data_downsampled_dic={}
    audio_files = os.listdir(recordings_dir) 
    for i,file in enumerate(audio_files):
        ...
        audio_data_downsampled_dic[i] = audio_data_downsampled
        # sd.play(audio_data_downsampled, samplerate=downsampling_new_sr)
        # sd.wait()
    noise_data_downsampled_dic={}
    noise_files = os.listdir(noise_dir) 
    for i,file in enumerate(noise_files):
        noise_path=os.path.join(noise_dir,file)
        noise_data, sample_rate = librosa.load(noise_path,sr=None)
        noise_data_downsampled = librosa.resample(noise_data,orig_sr=sampling_freq,target_sr=downsampling_new_sr)
        noise_data_downsampled_dic[i] = noise_data_downsampled
        # sd.play(noise_data_downsampled, samplerate=downsampling_new_sr)
        # sd.wait()
    return audio_data_downsampled_dic, noise_data_downsampled_dic
def transfer_fun(audio_resampled):
    if use_filter == False:
        return audio_resampled
    else:
        audio_transformed = {}
        system = signal.TransferFunction(filter_num_coeff, filter_dem_coeff).to_discrete(1/downsampling_new_sr)
        for i in range (0, len(audio_resampled)):
            _,audio_transformed_single_rec = signal.dlsim(system,audio_resampled[i])
            audio_transformed[i] = audio_transformed_single_rec
        return audio_transformed
def gain_fun (audio, noise):
    audio_desired_gain_linear = 10 ** (audio_gain / 20)
    noise_desired_gain_linear = 10 ** (noise_gain / 20)

    adjusted_audio = {}
    for i in range (0, len(audio)):
        adjusted_audio_single_rec = audio[i] * audio_desired_gain_linear
        adjusted_audio[i] = adjusted_audio_single_rec

    adjusted_noise = {}
    for i in range (0, len(noise)):
        adjusted_noise_single_rec = noise[i] * noise_desired_gain_linear
        adjusted_noise[i] = adjusted_noise_single_rec
...
def run_CNN():
    ...
    # with open(r"C:\Users\mn1059928\OneDrive - Bose Corporation\Documents\GitHub\CNN_Git\input_data.pkl","rb") as file:
